...
async function storageList(bucket){
  const res=await fetch(`${SUPABASE_URL}/storage/v1/object/list/${bucket}`,{method:'POST',headers:{apikey:SUPABASE_KEY,Authorization:`Bearer ${SUPABASE_KEY}`,'Content-Type':'application/json'},body:JSON.stringify({prefix:'',limit:1000,offset:0,sortBy:{column:'name',order:'asc'}})});
  const text=await res.text();
  if(!res.ok) throw new Error(`Storage list ${res.status}: ${text.slice(0,500)}`);
  return text?JSON.parse(text):[];
}
async function storageUpload(bucket,name,filePath){
  const data=fs.readFileSync(filePath);
  const res=await fetch(`${SUPABASE_URL}/storage/v1/object/${bucket}/${encodeURIComponent(name)}`,{method:'POST',headers:{apikey:SUPABASE_KEY,Authorization:`Bearer ${SUPABASE_KEY`,'Content-Type':'application/octet-stream','x-upsert':'true'},body:data});
  const text=await res.text();
  if(!res.ok) throw new Error(`Storage upload ${res.status}: ${text.slice(0,300)}`);
}
async function storageDownload(bucket,name,filePath){
  const res=await fetch(`${SUPABASE_URL}/storage/v1/object/${bucket}/${encodeURIComponent(name)}`,{headers:{apikey:SUPABASE_KEY,Authorization:`Bearer ${SUPABASE_KEY}`} });
  if(!res.ok) throw new Error(`Storage download ${res.status} ${bucket}/${name}`);
...
async function syncBucket(bucket){
  const dir=bucketLocalDir(bucket); fs.mkdirSync(dir,{recursive:true});
  const remote=(await storageList(bucket)).filter(x=>x && x.name && !x.id?.endsWith('/'));
  const remoteNames=new Set(remote.map(x=>x.name));
  for(const obj of remote){ const local=path.join(dir,obj.name); if(!fs.existsSync(local)) await storageDownload(bucket,obj.name,local); }
  for(const name of fs.readdirSync(dir,{withFileTypes:true}).filter(x=>x.isFile()).map(x=>x.name)){
    if(!remoteNames.has(name)) await storageUpload(bucket,name,path.join(dir,name));
  }
}
...
function key(table,row){ if(COMPOSITE[table]) return COMPOSITE[table].map(k=>String(row[k])).join(':'); return String(row.id); }
async function deleteMissing(table,oldRows,newRows){
  ...
  for(const old of oldRows){
    ...
    if(COMPOSITE[table]){ const [a,b]=COMPOSITE[table]; await request(table,{method:'DELETE',query:`?${a}=eq.${encodeURIComponent(old[a])}&${b}=eq.${encodeURIComponent(old[b])}`}); }
    else if(old.id!=null) await request(table,{method:'DELETE',query:`?id=eq.${encodeURIComponent(old.id)}`});
  }
}
async function sync(db,snapshot){
  for(const table of TABLES){ const current=Array.isArray(db[table])?db[table]:[]; const old=Array.isArray(snapshot[table])?snapshot[table]:[]; await deleteMissing(table,old,current); await upsert(table,current); }
...
async function main(){
  ...
  async function flush(){
    ...
    try{ const current=readLocal(); snapshot=await sync(current,snapshot); await syncStorage(); }
